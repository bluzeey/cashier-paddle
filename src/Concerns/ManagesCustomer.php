...
trait ManagesCustomer
{
    ...
    public function createAsCustomer(array $options = [])
   {
    ...
    // Check if the Paddle customer already exists in the local database
    $existingCustomer = Cashier::$customerModel::where('paddle_id', $response['id'])->first();
    if ($existingCustomer) {
        return $existingCustomer;
    }

    // Create and save the new customer in the local database
